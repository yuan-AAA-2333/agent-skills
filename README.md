...
| [`windows-bulk-relocation`](windows-bulk-relocation/SKILL.md) | Moving applications, data directories, caches or whole code trees to another drive while everything must keep working; relocating an agent's own runtime data; consolidating scattered projects; reclaiming space on `C:`. | The `copy 鈫?per-file verify 鈫?rename source 鈫?junction` pattern (never cut-paste); why a file count is not a verification; robocopy `/MOVE` skip traps; hardlink double-counting by inode; junction hygiene (case-aliased paths, `islink` vs `isjunction`, chains, the 11 OS compatibility junctions); what must never be moved; self-elevating handover scripts. |
| ↳ [scripts/](windows-bulk-relocation/scripts/README.md) | You want the tooling, not just the rules. | 17 parameterised, working scripts: the copy→verify→backup→junction loop, the per-file audit, tree diff/measure/inventory, cache reclamation, a self-elevating fixer, the reference cross-check harness, two credential sanitizers, and two software-copyright generators. |
...
> 鈿狅笍 If you have edited the installed copies, do not blindly re-copy: compare first
> (`Get-FileHash`) and merge 鈥?the installed copy can legitimately be *newer* than the repo.
...
- **"winget is hung"** 鈥?it wasn't. winget delegates the download to the DeliveryOptimization service, so
...
- **"IPv4 doesn't work here"** 鈥?the answer *flipped twice within one session* as the active link changed
  between a campus WLAN and a USB-tethered phone. Test both families every time; never hardcode the verdict.
- **`Face:b00c`** 鈥?a DNS answer in Meta's `2a03:2880::/32` range for `registry-1.docker.io`, complete with
  the `face:b00c` poisoning fingerprint. Docker's own error text blames "no HTTPS proxy", which is a
  misleading template 鈥?chasing a proxy is a dead end.
- **`sc config 鈥** 鈥?`sc` is a PowerShell alias for `Set-Content`, so the real `sc.exe` is never invoked
...
- **"`robocopy /MOVE` succeeded"** 鈥?it exited 1 (success) while silently skipping a pnpm symlink tree:
...
- **"the relaxation is the objective"** 鈥?it isn't. A QUBO linearisation scored a solution at energy 5.0
  ...
  ground truth 鈥?not by reading the code.
- **"the last page is fine"** 鈥?a 3-line final page in a 65-page source listing gets a 杞憲 filing
  rejected. The registry wants 鈮?0 lines on *every* page.

## License

No license file yet 鈥?add one if you intend to reuse this. Until then, default copyright applies.
