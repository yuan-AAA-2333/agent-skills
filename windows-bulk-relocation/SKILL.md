...
description: Use when moving applications, data directories, caches, or whole code trees to another drive on Windows while everything must keep working 鈥?relocating an agent's own data, migrating program installs, consolidating scattered projects onto one disk, or reclaiming space on C:. Covers copy鈫抳erify鈫抝unction (never cut-paste), the audit that catches silent file loss, robocopy /MOVE traps, hardlink double-counting, and the elevation cases that need a self-elevating handover script.
---
...
```
copy (robocopy /E /COPY:DAT)  鈫? per-file verify  鈫? rename source into _backup/  鈫? mklink /J at the old path
```
...
The source is *renamed*, never deleted, until verification has passed 鈥?that rename is the rollback.
...
## Rule 1 鈥?a file count is not a verification
...
| A "35 GB of reclaimable cache" estimate was wrong by 12脳 | `Get-ChildItem 鈥?\| Measure-Object Length -Sum` double-counts hardlinks |
...
# /MOV not /MOVE 鈥?moves files, then delete the empty skeleton yourself:
...
## Rule 2 鈥?hardlinks inflate every "size" number
...
- **Python's `os.path.islink()` returns False for junctions** 鈥?it only sees symlinks. Use
...
- **Windows ships 11 compatibility junctions** in `%USERPROFILE%` (`Application Data`, `My Documents`,
  `Cookies`, `Local Settings`鈥?. They have no `Target` readable via `.Target`; they are not yours and
...
## Rule 4 鈥?the process you are running inside is not movable

A DSH/agent session executing from `鈥npm-cache\_npx\<hash>\` cannot relocate that directory: open file
handles cause `ERROR 5` on delete and the session would be severed. Detect and skip it explicitly 鈥?treat "source is in use" as an expected outcome with a documented alternative, not a failure to retry.
...
## Rule 5 鈥?elevation is not negotiable, so hand over a script
...
When elevation is needed, ship a **self-elevating `.ps1` plus an ASCII-only `.cmd` launcher** 鈥?not a
paste block. `.cmd` must be pure ASCII (cmd.exe decodes it as the ANSI codepage).

## Rule 6 鈥?audit afterwards, and record the exceptions
...
- the live tree legitimately **grew** (sessions/config written after the snapshot) 鈫?`extra > 0`
...
**not** touched, and the one-line rollback for each unit (delete junction 鈫?rename backup back).

> ## Reusable artifact

This skill ships its tooling in [scripts/](scripts/) — 17 parameterised scripts, see
[scripts/README.md](scripts/README.md) for what each one is for. Highlights: Move-DataUnit.ps1`n(the whole copy→verify→backup→junction loop), Audit-Migration.ps1 (the per-file audit),
Reclaim-More-Space.ps1, Fix-Modex-Admin.ps1 (self-elevating). **Edit the in-file unit table
before running** — the paths are kept as concrete examples from the original machine.