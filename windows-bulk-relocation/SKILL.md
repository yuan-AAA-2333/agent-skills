...
`E:\dsh\migration\scripts\` contains working versions of all of it: `Move-DataUnit.ps1` (copy→verify→
backup→junction with a JSONL log and idempotent re-runs), `Move-CodeUnit.ps1`, `Audit-Migration.ps1`,
`Compare-Tree.ps1`, `Measure-Tree.ps1`, `Reclaim-More-Space.ps1`, `Fix-Modex-Admin.ps1` (self-elevating).
